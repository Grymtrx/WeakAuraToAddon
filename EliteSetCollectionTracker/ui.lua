...
local function LayoutIcons(order)
    ...
    local width = 16 + columns * ICON_SIZE + (columns - 1) * ICON_SPACING
    ...
    for idx, classToken in ipairs(order) do
        ...
        if icon then
            ...
            icon:SetPoint("TOPRIGHT", frame, "TOPRIGHT", -8 - col * (ICON_SIZE + ICON_SPACING), -24 - row * (ICON_SIZE + ICON_SPACING))
        end
    end
end
...
function ns.UpdateDisplay()
    ...
    local sorted = {}
    for _, classToken in ipairs(collectedOrder) do
        sorted[#sorted + 1] = classToken
    end
    for _, classToken in ipairs(missingOrder) do
        sorted[#sorted + 1] = classToken
    end
    for _, classToken in ipairs(unknownOrder) do
        sorted[#sorted + 1] = classToken
    end
